  ...
  <v-container class="py-16 my-16">
    <v-row justify="center gutter">
      <v-card v-for="(distribuidor, i) in distribuidores" :key="i" class="mx-3 mb-6" max-width="344">
        ...
        <v-card-actions>
          <v-btn v-if="distribuidor.whatsapp" text color="#00C853" :href="`https://api.whatsapp.com/send?phone=55${distribuidor.ddd + distribuidor.whatsapp}${$route.params.produto ? '&text=Olá, eu gostaria de saber mais sobre o produto id:'+$route.params.produto:''}`" target="_blank"><v-icon size="14px" class="mr-1">mdi-whatsapp</v-icon> WhatsApp</v-btn>
